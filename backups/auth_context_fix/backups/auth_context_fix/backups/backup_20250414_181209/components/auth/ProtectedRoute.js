...
export default function ProtectedRoute({ children, professionalOnly = false }) {
  const { isAuthenticated, user?.is_professional || user?.user?.is_professional || user?.user?.is_professional || user?.user?.is_professional || user?.user?.is_professional || user?.isProfessional, isLoading } = useAuth();
  ...
  useEffect(() => {
    ...
    if (!isLoading && isAuthenticated && professionalOnly && !user?.is_professional || user?.user?.is_professional || user?.user?.is_professional || user?.user?.is_professional || user?.user?.is_professional || user?.isProfessional) {
      router.push('/client/dashboard');
    }
  }, [isAuthenticated, user?.is_professional || user?.user?.is_professional || user?.user?.is_professional || user?.user?.is_professional || user?.user?.is_professional || user?.isProfessional, isLoading, router, professionalOnly]);
